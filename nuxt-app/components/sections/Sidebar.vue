<template>
  <header id="sidebar_header" class="lg:relative lg:translate-x-0 absolute transform custom-scrollbar"
    ...
    <div>
      ...
      <div class="flex justify-center items-center mt-4 cursor-pointer">
        <!-- Switch Container -->
        <div class="w-14 h-7 flex items-center rounded-full mx-3 px-1"
          :class="`${isDarkMode ? 'bg-primary' : 'bg-navy'}`" @click="$emit('toggleDarkMode')">
          <!-- Switch -->
          <div class="bg-white w-5 h-5 rounded-full shadow-md transform transition-transform duration-300"
            :class="{ 'translate-x-7': isDarkMode }"></div>
        </div>
      </div>
    </div>
  </header>
